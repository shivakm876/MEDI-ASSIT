...
def load_model_and_data():
    global model, symptoms_dict, diseases_list
    
    if model is None:
        # Load the model
        model_path = Path(__file__).parent.parent / 'public' / 'models' / 'svc.pkl'
        with open(model_path, 'rb') as f:
            model = pickle.load(f)
        
        # Create and save symptoms dictionary
        symptoms_dict = {
...
def get_disease_details(disease):
    datasets_path = Path(__file__).parent.parent / 'public' / 'datasets'
    
    # Load description
    description_df = pd.read_csv(datasets_path / 'description.csv')
    description = description_df[description_df['Disease'] == disease]['Description'].iloc[0]
    
    # Load precautions
    precautions_df = pd.read_csv(datasets_path / 'precautions_df.csv')
    precautions = precautions_df[precautions_df['Disease'] == disease].iloc[0].tolist()[1:]
    precautions = [p for p in precautions if pd.notna(p)]
    
    # Load medications
    medications_df = pd.read_csv(datasets_path / 'medications.csv')
    medications_str = medications_df[medications_df['Disease'] == disease]['Medication'].iloc[0]
    medications = json.loads(medications_str.replace("'", '"'))
    
    # Load diet
    diets_df = pd.read_csv(datasets_path / 'diets.csv')
    diet_str = diets_df[diets_df['Disease'] == disease]['Diet'].iloc[0]
    diet = json.loads(diet_str.replace("'", '"'))
    
    # Load workout
    workout_df = pd.read_csv(datasets_path / 'workout_df.csv')
...
if __name__ == '__main__':
    ...
    if sys.argv[1] == '--load-only':
        # Just load the model and return success
        load_model_and_data()
        ...
        try:
            ...
            result = get_disease_details(disease)
